...
What does this mean?
My Answer: At 95% confidence, the average rent in Manhattan ranges from $2,299
to $2,977.
...
Answer: At 95% confidence, the median rent in Manhattan ranges from $2,162 to
$2,875.
...
What does this mean?
Answer: At 95% confidence, the proportion of mice that died while not on the
treatment ranges from 0.690 to 0.797.
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(p2_boot_prop, level = .9)
ci_mice_lived
```
...
visualise(p2_boot_prop) +
  shade_ci(ci_mice_lived)
```


Give an interpretation of your result.
Answer: At 95% confidence, the proportion of mice that live while on the
treatment ranges from 0.552 to 0.651.
